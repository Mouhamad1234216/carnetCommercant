...
if (!/nomodule/.test(newHtml)) {
  // Inject a nomodule fallback that points to the server-provided legacy-fallback.js with cache-busting.
  const ts = Date.now();
  newHtml = newHtml.replace(match[0], `${match[0]}\n    <script nomodule src="/assets/legacy-fallback.js?v=${ts}"></script>`);
  fs.writeFileSync(indexFile, newHtml, 'utf8');
  console.log('Injected nomodule fallback (/assets/legacy-fallback.js?v=TIMESTAMP) into dist/index.html and removed crossorigin attributes');
} else {
  fs.writeFileSync(indexFile, newHtml, 'utf8');
